...
build-development-images: ## Build the images
	nerdctl build --platform=amd64,arm64 --tag containers.chewed-k8s.net/${NAMESPACE}/${SERVICE_NAME}:${GIT_COMMIT} --build-arg VERSION=0.1 --build-arg BUILD=${GIT_COMMIT} --build-arg SERVICE_NAME=${SERVICE_NAME} --build-arg CONTAINER_ENV=development -f ./k8s/Containerfile.dev .
	nerdctl tag containers.chewed-k8s.net/${NAMESPACE}/${SERVICE_NAME}:${GIT_COMMIT} containers.chewed-k8s.net/${NAMESPACE}/${SERVICE_NAME}:latest
...
deploy-beta: publish-images
	kubectl set image deployment/${SERVICE_NAME} ${SERVICE_NAME}=containers.chewed-k8s.net/${NAMESPACE}/${SERVICE_NAME}:${GIT_COMMIT} --namespace=${NAMESPACE}-dev
...
build-deploy-dev: build-development-images publish-images deploy-dev
